...
import type { DatasetBlockFile, DatasetBlockFrontmatter } from "./_helpers";
import { formatBytes, formatDate, getCurrentFiles } from "./_helpers";
...
export function CurrentFiles({
  ...
  const files = getCurrentFiles(frontmatter);

  return (
    <section className="space-y-4">
      <div>
        <h2 className="text-lg font-semibold text-slate-900">Current Files</h2>
        <p className="mt-1 text-sm text-slate-600">
          Downloads for the current published dataset version.
        </p>
      </div>
